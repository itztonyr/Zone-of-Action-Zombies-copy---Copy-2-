Thanks for purchase Level System for MFPS 2.0
Version 1.5.2

Require:
 
ULogin Pro v1.9++

Get Started: --------------------------------------------------------------------------------------------------------------------

- Import the package in the MFPS 2.0 project with ULogin Pro already installed.
- in toolbar go to MFPS -> Addons -> LevelManager -> Enable
- also MFPS -> Addons -> LevelManager -> Integrate

Documentation: ---------------------------------------------------------------------------------------------------------------------------

Open the documentation at: MFPS -> Tutorials -> Level Manager

Runtime Level Notifier: ----------------------------------------------------------------------------------------------------------

In order to show when player level up in Match time, you have to integrate the notifier UI in the map scene (have to do for each map scene):

 1 - Open the map scene in the Editor 
 2 - Go to MFPS -> Addons -> LevelManager -> Integrate RLN
 3 - Save the scene.


Change Log:

1.5.2
Compatibility with MFPS 2.0 v1.10.0
Fix: Exception throw in the MainMenu scene when login as guest in ULogin Pro and Level Manager is enabled.
Fix: The runtime level up notification show the new level text in the wrong text component.

1.5.1
Improved: Now unlock list include emblems and calling card items.

1.5.0
Compatibility with MFPS 1.9
Add: Level Editor window tab in the MFPS Manager window (MFPS 1.9++)
Change: Now the level progression will be integrated in the 'Barraks' window.
Improve: Now the level progression information is modular and easily to modify.
Improve: Over all code, check public functions of bl_LevelManager and LevelInfo for custom usage.
Add: Items Unlock list in the lobby progress window.
Improve: Use Text Mesh Pro.
Improve: Now the level icon shows for bots too in the scoreboard.

1.4.2
Compatibility with Game Resume Pro addon.

Version 1.4
Improve: Rank icons.
Improve: Setup 78 levels with custom icons for each.
Improve: Lobby level progress window UI.
Improve: Now the level scroll list will automatically scroll to the current level of the player.

Version 1.3.5
Add: Runtime Level Notifier, now you can show when player level up in match.

Version 1.3 (6/6/2019)
Improve: Integration process.
Improve: Now players levels are update in mid game.

Version 1.2
Add: Level progress window in Lobby.