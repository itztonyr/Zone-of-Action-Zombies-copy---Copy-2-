...
2.5.0 (Full Update)
-Improved: Support for MFPS 1.10.0
-Improved: add abstractable integration with MFPS (1.10 and later) instead of integration with the MFPS core scripts.
-Fix: Account logout cache issue with the clan and player roles.
-Fix: Error when enable the AntiCheat Toolkit addon and have saved login credentials.
-Change: Now the PHP class names defined locally are placed in an array instead of separate variables.
-Improved: Better support ticket system, now store the whole conversation in the database.
-Improved: Now you can delete support tickets from the Admin Panel.
-Fix: PlayAsGuest button do not appear when logout from the account in the login scene.
-Improved: Update phpseclib and phpmailer libraries to the latest version.
-Improved: Now use composer to manage the PHP libraries.
