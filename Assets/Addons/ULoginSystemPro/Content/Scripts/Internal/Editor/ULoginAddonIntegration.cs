...
public class ULoginAddonIntegration : AddonIntegrationWizard
{
    ...
    {
        var saveHandler = AssetDatabase.LoadAssetAtPath("Assets/Addons/ULoginSystemPro/Content/Prefabs/Presets/ULogin Database Handler.asset", typeof(bl_DatabaseBase)) as bl_DatabaseBase;
        if (saveHandler != null)
        {
            bl_GlobalReferences.I.databaseHandler = saveHandler;
            EditorUtility.SetDirty(bl_GlobalReferences.I);
        }
        else
        {
            Debug.LogWarning("The ULogin Database Handler is missing, please add it manually.");
        }
