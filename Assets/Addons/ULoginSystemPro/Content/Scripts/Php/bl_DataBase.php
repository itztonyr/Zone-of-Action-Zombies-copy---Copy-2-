...
if (isset($_POST['data'])) {
    // check if data value start with ecr-
    if (substr($_POST['data'], 0, 4) == 'ecr-') {
        // remove ecr- from data
        $data = substr($_POST['data'], 4);
        $data = Utils::sanitaze_var($data, $link, $sid);
    } else {
...
function updateUserRows()
{
    ...
    if ($functions->update_user_data_safe($key, $values, 'id', $id, true))
        success_response();
    else
        fail_response();
}
...
/*
* Sum integer values from given keys
*/
function sumUserRows()
{
    global $functions;
    global $key;
    global $values;
    ...
    if ($functions->update_user_data_safe($key, $values, 'id', $id, false))
        success_response();
    else
        fail_response();
}
