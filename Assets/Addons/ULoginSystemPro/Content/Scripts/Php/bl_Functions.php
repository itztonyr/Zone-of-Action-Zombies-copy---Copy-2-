...
class Functions
{
    ...
    */
     ...
     * @return {boolean} true if the update query success, false if fail or the update values are the same than were before
    */
    public function update_user_data_safe($fields, $values, $where, $identifier, $override = true)
    {
        $inserts = $this->string_to_assoc_array($fields, $values);
        if (!is_array($inserts)) {
            die("Invalid format, couldn't parse fields and values.");
        }
        ...
        foreach (array_keys($inserts) as $key) {
            ...
            $isNumeric = filter_var($inserts[$key], FILTER_VALIDATE_INT) !== false;
            if ($isNumeric) {
                // replace the string value with the int value
                $inserts[$key] = (int)$inserts[$key];
            }
