...
public class bl_DataBase : bl_LoginProBase
{
    ...
    public void SaveValue(string key, object value, Action callBack = null, bool forceClearData = false)
    {
        ...
        if (!forceClearData) wf.AddSecureField("data", value.ToString(), "ecr-");
    ...
    public void UpdateUserData(ULoginUpdateFields fieldsToUpdate, Action<bool> callback = null)
    {
        ...
        WebRequest.POST(GetURL("database"), wf, (result) =>
        {
            if (result.isError)
            {
                result.PrintError();
    ...
    public void UpdateUserCoins(int coins, int coinID, ULoginCoinsOp coinsOp, Action callback = null)
    {
        ...
        int currentCoins = bl_MFPS.Coins.GetCoinData(coinID).GetCoins();
        ...
        WebRequest.POST(GetURL("database"), wf, (result) =>
        {
            ...
            if (result.resultState == ULoginResult.Status.Success)
            {
                ...
                if (int.TryParse(result.Text, out newCoins))
                {
                    ...
                    Debug.LogFormat($"User coins updated, the new total is {newCoins}, before {currentCoins}, difference {currentCoins - newCoins}");
    ...
    public void SetClanScore(int newScore)
    {
        ...
        WebRequest.POST(GetURL("clan"), wf, (r) =>
          {
              if (r.isError)
              {
                  r.PrintError();
    ...
    public void SetCoinPurchase(CoinPurchaseData data, Action<bool> callback)
    {
        ...
        WebRequest.POST(bl_LoginProDataBase.GetUrl("database"), wf, (result) =>
        {
            result.Print();
            if (result.resultState == ULoginResult.Status.Success)
            {
    ...
    void BanComprobation()
    {
        ...
        WebRequest.POST(GetURL("banlist"), wf, (result) =>
          {
              if (result.isError)
              {
                  result.PrintError();
    ...
    public static void DeleteAccount(int userId, string loginName, Action<bool> callback)
    {
        ...
        Instance.WebRequest.POST(bl_LoginProDataBase.GetUrl("account"), wf, (r) =>
        {
            if (r.isError)
            {
                callback?.Invoke(false);
    ...
    public string DataBaseURL => bl_LoginProDataBase.GetUrl("database");
