...
namespace MFPS.ULogin
{
    public class bl_NickNamePanel : MonoBehaviour
    {
        ...
        [SerializeField] private TMP_InputField nickInputfield = null;
        private Action<string> onSet;

        public void SetCallback(Action<string> callback, string description = "", string defaultNick = "")
        {
            if (!string.IsNullOrEmpty(description))
            {
                descriptionText.text = description;
            }
            if (nickInputfield != null) { nickInputfield.text = defaultNick; }
            onSet = callback;
        }
