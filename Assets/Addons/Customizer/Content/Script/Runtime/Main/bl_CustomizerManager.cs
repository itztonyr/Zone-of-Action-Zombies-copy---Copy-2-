...
namespace MFPS.Addon.Customizer
{
    public class bl_CustomizerManager : MonoBehaviour
    {
        ...
        public bl_WorldWeaponsContainer tpWeaponContainer;
        ...
        private readonly List<GameObject> cacheButtons = new List<GameObject>();
        ...
        void Awake()
        {
            ...
            int indexOfWeapon = AllCustom.FindIndex(x => x.WeaponID == savedWeaponID);
            ShowCustomizerWeapon(AllCustom[indexOfWeapon]);

            CustomizerButtonPrefab.SetActive(false);
            AttachButtonPrefab.SetActive(false);
            InstanceCustomizers(indexOfWeapon);
            bl_Input.Initialize();
        ...
        /// 
        /// </summary>
        void InstanceCustomizers(int firstSelected)
        {
            for (int i = 0; i < AllCustom.Count; i++)
            {
                ...
                if (i == firstSelected) { g.GetComponent<bl_CustomizerInfoButton>().OnSelect(); }
            }
        }
